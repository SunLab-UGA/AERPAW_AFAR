...
# set this to True to show the mean squared error (MSE) instead of the mean prediction
SHOW_MSE = False 

#list the files in the folder
print('found files in folder: ', os.listdir(folder))
...
def update(frame_number, zarray):
    ...
    ax.clear()
    z = zarray[frame_number] # mean prediction
    if SHOW_MSE: # show the mean squared error (MSE) instead of the mean prediction
        z = data['Z_mse_history'][frame_number] # mse prediction
    z = np.reshape(z, (ugv_grid_step, ugv_grid_step))

    cc = data['Z_history_uav'][frame_number] # mean prediction
    if SHOW_MSE: # show the mean squared error (MSE) instead of the mean prediction
        cc = data['Z_mse_history_uav'][frame_number] # mse prediction
    cc = np.reshape(cc, (uav_grid_step, uav_grid_step))
