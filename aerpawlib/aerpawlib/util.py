...
def strip_altitudes(coordinates:List[Coordinate]):
    """
    Returns a list of 2d coordinates from a list of 3d coordinates
    """
    d2 = []
...
def write_coordinates_to_file(coordinates:list[Coordinate], filename):
    '''
    Writes coordinates to kml file
    '''
    # check if coordinates is a list of coordinates and/or has at least one coordinate
    if not isinstance(coordinates, list):
        raise TypeError("Coordinates must be a list of coordinates")
    if len(coordinates) == 0:
        raise ValueError("Coordinates must have at least one coordinate")
    
    kmls = []
    for i,c in enumerate(coordinates):
        name = KML.name(f"{i}")
        point = KML.Point(KML.coordinates(f"{c.lon},{c.lat}"))
        mark = KML.Placemark(name, point)
        kmls.append(mark)
    fld = KML.Folder(*kmls) # put the points in a folder
    # output to file
    # print(etree.tostring(fld, pretty_print=True)) # verbose
